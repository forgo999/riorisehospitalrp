...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/covenants", requireAuth, requireAdminOrDirector, async (req: AuthRequest, res) => {
    try {
  ...
  app.patch("/api/covenants/:id", requireAuth, requireAdminOrDirector, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      ...
      await storage.createLog({
        ...
        userId: req.currentUser!.id,
        shiftId: null,
        details: `Convênio atualizado: ${covenant.organizationName}`
      });
  ...
  app.delete("/api/covenants/:id", requireAuth, requireAdminOrDirector, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const covenant = await storage.getCovenant(id);
  ...
  app.post("/api/me-commands", requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      const permission = canManageResource(currentUser.role, currentUser.shiftId, commandData.type, commandData.shiftId);
      if (!permission.allowed) {
        return res.status(403).json({ message: permission.message });
      }
      ...
      await storage.createLog({
        action: LogAction.CREATE_ME_COMMAND,
        userId: currentUser.id,
        shiftId: commandData.shiftId,
        details: `Comando /me criado: ${command.text}`
      });
  ...
  app.patch("/api/me-commands/:id", requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      const permission = canManageResource(currentUser.role, currentUser.shiftId, existingCommand.type, existingCommand.shiftId);
      if (!permission.allowed) {
        return res.status(403).json({ message: permission.message });
      }
      ...
      await storage.createLog({
        action: LogAction.UPDATE_ME_COMMAND,
        userId: currentUser.id,
        shiftId: command!.shiftId,
        details: `Comando /me atualizado: ${command!.text}`
      });
  ...
  app.delete("/api/me-commands/:id", requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      await storage.createLog({
        action: LogAction.DELETE_ME_COMMAND,
        userId: currentUser.id,
        shiftId: existingCommand.shiftId,
        details: `Comando /me deletado: ${existingCommand.text}`
      });
  ...
  app.get("/api/me-categories/general", async (req, res) => {
    try {
      const categories = await storage.getMeCategoriesByType("general");
      res.json(categories);
    } catch (error) {
      res.status(500).json({ message: "Erro ao buscar categorias gerais" });
    }
  });

  app.get("/api/me-categories/shift/:shiftId", async (req, res) => {
    try {
      const { shiftId } = req.params;
      const categories = await storage.getMeCategoriesByShift(shiftId);
      res.json(categories);
    } catch (error) {
      res.status(500).json({ message: "Erro ao buscar categorias do turno" });
    }
  });
  ...
  app.post("/api/me-categories", requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      const permission = canManageResource(currentUser.role, currentUser.shiftId, categoryData.type, categoryData.shiftId);
      if (!permission.allowed) {
        return res.status(403).json({ message: permission.message });
      }
      ...
      await storage.createLog({
        action: LogAction.CREATE_ME_CATEGORY,
        userId: currentUser.id,
        shiftId: categoryData.shiftId,
        details: `Categoria /me criada: ${category.name}`
      });
  ...
  app.patch("/api/me-categories/:id", requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      const permission = canManageResource(currentUser.role, currentUser.shiftId, existingCategory.type, existingCategory.shiftId);
      if (!permission.allowed) {
        return res.status(403).json({ message: permission.message });
      }
      ...
      await storage.createLog({
        action: LogAction.UPDATE_ME_CATEGORY,
        userId: currentUser.id,
        shiftId: category!.shiftId,
        details: `Categoria /me atualizada: ${category!.name}`
      });
  ...
  app.delete("/api/me-categories/:id", requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      await storage.createLog({
        action: LogAction.DELETE_ME_CATEGORY,
        userId: currentUser.id,
        shiftId: existingCategory.shiftId,
        details: `Categoria /me deletada: ${existingCategory.name}`
      });
