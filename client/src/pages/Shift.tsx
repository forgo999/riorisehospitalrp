...
import { Calendar, Users, FileText, MessageSquare, User } from "lucide-react";
import { type Shift as ShiftType, type Rule, type MeCommand, type User as UserType, UserRole } from "@shared/schema";
...
export default function Shift() {
  ...
  const { data: commands = [] } = useQuery<MeCommand[]>({
    queryKey: user?.shiftId ? [`/api/me-commands/shift/${user.shiftId}`] : ["/api/me-commands"],
  });
  ...
  const shiftCommands = commands.filter(c => c.shiftId === user?.shiftId);
      ...
      {shiftCommands.length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold">
              Comandos /me do Turno ({shiftCommands.length})
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {shiftCommands.map((command) => (
              <MeCommandCard key={command.id} command={command} />
            ))}
          </div>
        </section>
      )}
