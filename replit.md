...
Hospital Rio Rise is a role-playing game management system for a fictional hospital. The application manages covenants (insurance agreements), shifts, rules, members, attendance tracking, disciplinary warnings, promotions, and role-playing commands ("/me" commands). It features role-based access control with seven user levels: Administrator, Director, Vice-Director, Surgeon (with Chief Surgeon variant), Therapist, Paramedic, and Intern. The system includes real-time countdown timers for covenant expiration, attendance roll call management, warning issuance system with auto-exoneration (3 warnings), comprehensive admin panel, promotion system, complete audit logs, and password-protected administrative actions. All data is stored in MongoDB.
...
- IStorage interface implementation with MongoDBStorage class
- Collections: users, shifts, covenants, rules, mecategories, mecommands, attendancerecords, warnings, promotions, logs
...
1. **Administrator (administrador):** Highest level access, can manage everything across all shifts, view all logs
2. **Director (diretor):** Full access to all shifts and administrative functions, view all logs
3. **Vice-Director (vice_diretor):** Can manage users in their shift only, promote up to Surgeon level, issue warnings/exonerate
4. **Surgeon (cirurgiao):** Can be regular or Chief Surgeon (1 chief per shift), view-only access to assigned shift
5. **Therapist (terapeuta):** View-only access to assigned shift
6. **Paramedic (paramedico):** View-only access to assigned shift
7. **Intern (estagiario):** Entry-level, view-only access to assigned shift
...
- Role-based permissions (7 level enum)
- Optional shift assignment
- Chief Surgeon flag (isChiefSurgeon) - limited to 1 per shift
...
**MeCategory Schema:**
- Dynamic categories for /me commands (Reception, Medication, Surgery, SOS, etc.)
...
**Warning Schema:**
- User and shift association
- Warning reason and occurrence type
- Issued by (user ID) tracking
- Auto-exoneration: 3 warnings = user deleted from system
- Created timestamp

**Promotion Schema:**
- User promotion history
- From role and to role tracking
- Promoted by (user ID) tracking
- Optional notes
- Created timestamp

**Log Schema:**
- Complete audit trail of all system actions
- Action types: login, create/update/delete operations, promotions, exonerations
- User and target user tracking
